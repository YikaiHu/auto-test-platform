...
const ServiceLogDetail: React.FC = () => {
  ...
  const getPipelineById = async () => {
    try {
      ...
      let tmpLogLocation = "";
      if (
        dataPipeline.type === ServiceType.S3 ||
        dataPipeline.type === ServiceType.CloudTrail ||
        dataPipeline.type === ServiceType.CloudFront ||
        dataPipeline.type === ServiceType.ELB ||
        dataPipeline.type === ServiceType.WAF ||
        dataPipeline.type === ServiceType.VPC ||
        dataPipeline.type === ServiceType.Config
      ) {
      ...
      setCurPipeline({
        ...
        stackId: dataPipeline.stackId ?? "",
        monitor: {
          status: dataPipeline?.monitor?.status,
          backupBucketName: "",
          errorLogPrefix: "",
          pipelineAlarmStatus: dataPipeline.monitor?.pipelineAlarmStatus,
          snsTopicName: dataPipeline.monitor?.snsTopicName || "",
          snsTopicArn: dataPipeline.monitor?.snsTopicArn || "",
          emails: dataPipeline.monitor?.emails || "",
        },
      });
              ...
              <div>
                ...
                <TabPanel value={activeTab} index={2}>
                  {isLightEngine ? (
                    ...
                    <LogProcessor pipelineInfo={curPipeline} />
                  )}
                </TabPanel>
