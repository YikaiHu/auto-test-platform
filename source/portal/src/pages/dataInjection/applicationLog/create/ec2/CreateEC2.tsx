...
import { buildOSIParamsValue, checkIndexNameValidate } from "assets/js/utils";
...
const AppLogCreateEC2: React.FC = () => {
  ...
  const selectProcessor = useSelectProcessor();
  ...
  const selectProcessorValidator = new Validator(() => {
    if (!validateOCUInput(selectProcessor)) {
      dispatch({
  ...
  const stepComps = [
    ...
    },
    {
      ...
      disabled: !!state,
      element: <SelectLogProcessor engineType={engineType} />,
  ...
  const confirmCreatePipeline = async (isForce = false) => {
    ...
    const createPipelineParams: CreateAppPipelineMutationVariables = {
      ...
      osiParams: buildOSIParamsValue(selectProcessor),
      tags,
      force: isForce,
    };
  ...
  const isNextDisabled = () => {
    return (
      ...
      selectProcessor.serviceAvailableCheckedLoading
    );
  };
